    ...
    extends TestSupport
{
  ...
  @Mock
  private OrientKeyStoreStorageManager storageManager;
  ...
  public void testExists() {
    ...
    when(storageManager.load(KEY_STORE_NAME)).thenReturn(new OrientKeyStoreData());
  ...
  public void testOnKeyStoreDataUpdated() throws Exception {
    ...
    assertThat(storage.modified(), is(false));
    storage.onKeyStoreDataUpdated(new OrientKeyStoreDataUpdatedEvent(newEntityMetadata("2"), "another" + KEY_STORE_NAME));
    assertThat(storage.modified(), is(false));
    storage.onKeyStoreDataUpdated(new OrientKeyStoreDataUpdatedEvent(newEntityMetadata("1"), KEY_STORE_NAME));
    assertThat(storage.modified(), is(false));
    storage.onKeyStoreDataUpdated(new OrientKeyStoreDataUpdatedEvent(newEntityMetadata("2"), KEY_STORE_NAME));
    assertThat(storage.modified(), is(true));
  }
  ...
  public void testSave() throws Exception {
    ...
    assertThat(storage.modified(), is(false));
    ArgumentCaptor<OrientKeyStoreData> entityCaptor = ArgumentCaptor.forClass(OrientKeyStoreData.class);
    verify(storageManager).save(entityCaptor.capture());
    OrientKeyStoreData entity = entityCaptor.getValue();
