 ...
 * @since 3.1
 */
@Named(OrientPyPiProxyRecipe.NAME)
@Singleton
class OrientPyPiProxyRecipe
    extends OrientPyPiRecipeSupport
{
  public static final String NAME = 'pypi-proxy'

  @Inject
  Provider<OrientPyPiProxyFacetImpl> proxyFacet
  ...
  @Inject
  OrientPyPiProxyRecipe(@Named(ProxyType.NAME) final Type type, @Named(PyPiFormat.NAME) final Format format) {
    super(type, format)
  }
