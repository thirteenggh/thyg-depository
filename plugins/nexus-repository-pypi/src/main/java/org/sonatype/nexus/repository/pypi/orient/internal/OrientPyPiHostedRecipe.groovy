 ...
 * @since 3.1
 */
@Named(OrientPyPiHostedRecipe.NAME)
@Singleton
class OrientPyPiHostedRecipe
    extends OrientPyPiRecipeSupport
{
  public static final String NAME = 'pypi-hosted'

  @Inject
  Provider<OrientPyPiHostedFacet> hostedFacet

  @Inject
  Provider<OrientPyPiComponentMaintenance> componentMaintenanceFacet

  @Inject
  Provider<PyPiIndexFacet> indexFacet

  @Inject
  OrientPyPiHostedHandlers hostedHandlers

  @Inject
  OrientPyPiHostedRecipe(@Named(HostedType.NAME) final Type type, @Named(PyPiFormat.NAME) final Format format) {
    super(type, format)
  }
