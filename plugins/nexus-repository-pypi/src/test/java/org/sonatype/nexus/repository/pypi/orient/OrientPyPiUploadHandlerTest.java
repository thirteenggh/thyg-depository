    ...
    extends TestSupport
{
  ...
  private OrientPyPiUploadHandler underTest;

  @Mock
  private Repository repository;

  @Mock
  private OrientPyPiHostedFacetImpl pyPiFacet;
  ...
  public void setup() throws IOException {
    ...
    underTest = new OrientPyPiUploadHandler(contentPermissionChecker, new SimpleVariableResolverAdapter(), emptySet());
    when(repository.facet(OrientPyPiHostedFacet.class)).thenReturn(pyPiFacet);
    ...
    when(tempBlob.get()).thenAnswer(invocation -> OrientPyPiUploadHandlerTest.class.getResourceAsStream("internal/sample-1.2.0-py2.7.egg"));
    when(storageFacet.createTempBlob(payload, HASH_ALGORITHMS)).thenReturn(tempBlob);
  ...
  public void testGetDefinitionWithExtensionContributions() {
    ...
    underTest = new OrientPyPiUploadHandler(contentPermissionChecker, new SimpleVariableResolverAdapter(), getDefinitionExtensions());
    UploadDefinition def = underTest.getDefinition();
