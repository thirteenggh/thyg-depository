...
/**
 * An implementation of a {@link KeyStoreManager} that stores the trusted
 * certificates and
 * public/private key used for authentication in different key-stores.
 *
 * Current key-store implementation appears to NOT to have case-sensitive alias
 * support (ie. "myKey" = "mykey").
 *
 * @since 3.0
 */
public class KeyStoreManagerImpl
  ...
  /**
   * Initializes the key-store with a default key (if not already created), so
   * that the identity key can be removed
   * w/o invalidating the re-loadable key-manager.
   * <p/>
   * This key is never used by anything. Its only here to prevent the key-store
   * from being empty of keys.
   */
  private KeystoreInstance initializePrivateKeyStore(final KeyStoreStorage storage) {
    ...
    if (!isKeyPairInstalled(ks, DEFAULT00_KEY_ALIAS)) {
      try {
        ...
        Certificate cert = ks.getCertificate(DEFAULT00_KEY_ALIAS, config.getPrivateKeyStorePassword());
  ...
  private KeystoreInstance initializeTrustedKeyStore(final KeyStoreStorage storage) {
    ...
    // FIXME: Log a warning for edge cases when truststore is not empty, it should
    // be (we manage this content via capability),
  ...
  public TrustManager[] getTrustManagers() throws KeystoreException {
    ...
    try {
      ...
      // important! any time we get the array of trust managers we need to replace the
      // X509TrustManager with the
      // ReloadableX509TrustManager so that changes to the keystore are updated in the
      // TrustManager
      try {
        reloadableX509TrustManager = ReloadableX509TrustManager.replaceX509TrustManager(reloadableX509TrustManager,
            trustManagers);
  ...
  public KeyManager[] getKeyManagers() throws KeystoreException {
    ...
    try {
      ...
      // important! any time we get the array of key managers we need to replace the
      // X509KeyManager with the
      // ReloadableX509KeyManager so that changes to the keystore are updated in the
      // KeyManager
      try {
        reloadableX509KeyManager = ReloadableX509KeyManager.replaceX509KeyManager(reloadableX509KeyManager,
            keyManagers);
      } catch (NoSuchAlgorithmException e) {
  ...
  public Collection<Certificate> getTrustedCertificates() throws KeystoreException {
    ...
    try {
      ...
      for (String alias : aliases) {
        ...
        // FIXME: Work around some strange case not clear why, but alias is reported for
        // non-existent/removed certs
        if (cert == null) {
